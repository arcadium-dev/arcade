...
func (s LinksService) List(w http.ResponseWriter, r *http.Request) {
	...
	// Translate from asset links, to network links.
...
func (s LinksService) Get(w http.ResponseWriter, r *http.Request) {
	...
	link, err := s.Storage.Get(ctx, asset.LinkID(linkID))
	if err != nil {
		server.Response(ctx, w, err)
		return
	}
...
func (s LinksService) Create(w http.ResponseWriter, r *http.Request) {
	...
	link, err := s.Storage.Create(ctx, asset.LinkCreate{LinkChange: change})
	if err != nil {
		server.Response(ctx, w, err)
		return
	}
...
func (s LinksService) Update(w http.ResponseWriter, r *http.Request) {
	...
	link, err := s.Storage.Update(ctx, asset.LinkID(linkID), asset.LinkUpdate{LinkChange: change})
	if err != nil {
		server.Response(ctx, w, err)
		return
	}
...
func (s LinksService) Remove(w http.ResponseWriter, r *http.Request) {
	...
	err = s.Storage.Remove(ctx, asset.LinkID(linkID))
	if err != nil {
		server.Response(ctx, w, err)
		return
	}
...
func NewLinkFilter(r *http.Request) (asset.LinkFilter, error) {
	...
	if values := q["ownerID"]; len(values) > 0 {
		...
		if err != nil {
			return asset.LinkFilter{}, fmt.Errorf("%w: invalid ownerID query parameter: '%s'", errors.ErrBadRequest, values[0])
		}
	...
	if values := q["locationID"]; len(values) > 0 {
		...
		if err != nil {
			return asset.LinkFilter{}, fmt.Errorf("%w: invalid locationID query parameter: '%s'", errors.ErrBadRequest, values[0])
		}
	...
	if values := q["destinationID"]; len(values) > 0 {
		...
		if err != nil {
			return asset.LinkFilter{}, fmt.Errorf("%w: invalid destinationID query parameter: '%s'", errors.ErrBadRequest, values[0])
		}
		filter.DestinationID = asset.RoomID(destinationID)
	}

	if values := q["offset"]; len(values) > 0 {
		offset, err := strconv.Atoi(values[0])
		if err != nil || offset <= 0 {
			return asset.LinkFilter{}, fmt.Errorf("%w: invalid offset query parameter: '%s'", errors.ErrBadRequest, values[0])
		}
	...
	if values := q["limit"]; len(values) > 0 {
		...
		if err != nil || limit <= 0 || limit > asset.MaxLinkFilterLimit {
			return asset.LinkFilter{}, fmt.Errorf("%w: invalid limit query parameter: '%s'", errors.ErrBadRequest, values[0])
		}
...
func TranslateLinkRequest(l rest.LinkRequest) (asset.LinkChange, error) {
	...
	if len(l.Description) > asset.MaxLinkDescriptionLen {
