...
func (c Client) ListPlayers(ctx context.Context, filter assets.PlayerFilter) ([]*assets.Player, error) {
	...
	q := req.URL.Query()
	if filter.LocationID != assets.NilRoomID {
...
func playersResponse(body io.ReadCloser, failMsg string) ([]*assets.Player, error) {
	var playersResp rest.PlayersResponse
	if err := json.NewDecoder(body).Decode(&playersResp); err != nil {
		return nil, fmt.Errorf("%s: %w", failMsg, err)
	}
	var aPlayers []*assets.Player
...
func playerResponse(body io.ReadCloser, failMsg string) (*assets.Player, error) {
